...
class Response:
    ...
    def create_temperature_dataframe(self) -> pd.DataFrame:
        return pd.DataFrame.from_dict({'datetime': [datetime.fromtimestamp(self.dt).strftime("%Y-%m-%d %H:%M:%S")],
...
class ResponseProcessor:
    ...
    def process(raw_response: str) -> Response:
        try:
            ...
            response: Response = Response(loaded_response.get('coord', {}).get('lat'),
