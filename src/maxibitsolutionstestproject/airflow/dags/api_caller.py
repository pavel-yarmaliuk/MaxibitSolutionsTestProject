...
class WeatherAPICaller:
    ...
    def __init__(self, latitude, longitude):
        ...
        self.__url = (f'https://api.openweathermap.org/data/2.5/weather?'
                      ...
                      f'&appid={self.__api_key}&units=metric')
    ...
    def make_api_call(self) -> str:
        try:
            ...
            LOGGER.info('HTTP Exception when making request to API, failed with message: %s',
                        http_exception)
            raise
        except requests.exceptions.ConnectionError as connection_exception:
            LOGGER.info('Exception in connection process failed with message: %s',
                        connection_exception)
            raise
        except requests.exceptions.Timeout as timeout_exception:
            LOGGER.info('Timeout Exception when connecting to API, failed with message: %s',
                        timeout_exception)
            raise
        except requests.exceptions.RequestException as request_exception:
            LOGGER.info('Exception when making request to API, failed with message: %s',
                        request_exception)
            raise
        return response
